...
function QuestionRow({
    ...
    onUpdate: (i: number, config: Partial<{ type: QuestionType; count: number; marksPerQuestion: number }>) => void;
...
export default function CreateAssignmentPage() {
  ...
  setGenerating(true);
  setProgress(3);
  setProgressMsg("Waking up server...");
  setSubmitting(true);

  // ✅ Progress continuously badhta rahe
  let currentProgress = 3;
  const progressTimer = setInterval(() => {
    currentProgress = Math.min(currentProgress + 0.5, 45);
    setProgress(Math.round(currentProgress));
    
    if (currentProgress < 10) setProgressMsg("Waking up server...");
    else if (currentProgress < 20) setProgressMsg("Server is ready, creating assignment...");
    else if (currentProgress < 35) setProgressMsg("Sending data to AI...");
    else setProgressMsg("AI is preparing your question paper...");
  }, 800);

  try {
    // ✅ Step 1: Ping server first to wake it up
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/health`);
      setProgress(15);
      setProgressMsg("Server awake! Creating assignment...");
    ...
    setProgress(50);
    setProgressMsg("Assignment created! AI generating questions...");
    setSubmitting(false);
    ...
    const poll = async () => {
      ...
      try {
        ...
        // Progress 50 → 95 during AI generation
        setProgress(p => Math.min(p + 2, 95));

        if (assignment.status === "processing") {
          setProgressMsg("AI is writing your questions...");
        } else {
          setProgressMsg("Worker processing your request...");
        }
    ...
    // Agar timeout hai toh bhi check karo — paper ban gaya hoga
    if (msg.includes("timeout")) {
      toast.error("Server slow hai. Checking if assignment was created...");
      
      // Try to find recently created assignment
      setTimeout(async () => {
        try {
          const assignments = await assignmentService.getAll();
          const recent = assignments[0]; // Latest assignment
          if (recent && (recent.status === "completed" || recent.status === "processing" || recent.status === "pending")) {
            setAssignmentId(recent._id);
            setGenerating(true);
            setProgress(30);
            setProgressMsg("Found your assignment! Waiting for completion...");
            
            let attempts = 0;
            const poll = async () => {
              attempts++;
              if (attempts > 60) { setGenerating(false); router.push("/assignments"); return; }
              
              try {
                const a = await assignmentService.getById(recent._id);
                if (a.status === "completed" && a.paperId) {
                  setProgress(100);
                  toast.success("🎉 Paper generated!");
                  setTimeout(() => { reset(); router.push(`/assignments/${recent._id}`); }, 1000);
                  return;
                }
                if (a.status === "failed") { setGenerating(false); toast.error("Failed."); return; }
                setProgress(p => Math.min(p + 2, 95));
              } catch { }
              setTimeout(poll, 4000);
            };
...
//               fontSize: 12, fontWeight: 700, flexShrink: 0
//             }}>{step}</div>
//             <span style={{ fontSize: 12, fontWeight: current === step ? 600 : 400, color: current === step ? "#1a1a1a" : "#71717a" }}>
//               {step === 1 ? "Assignment Details" : "Review & Generate"}
//             </span>
//           </div>
//           {i === 0 && (
//             <div style={{ flex: 1, height: 2, margin: "0 12px", background: current >= 2 ? "#1a1a1a" : "#e4e4e7", borderRadius: 2 }} />
//           )}
//         </div>
//       ))}
//     </div>
...
//             style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "#ef4444", background: "none", border: "none", cursor: "pointer" }}>
//             <X size={12} /> Remove file
//           </button>
//         </>
//       ) : (
...
//             <Upload size={20} color="#71717a" />
//           </div>
//           <p style={{ fontSize: 13, fontWeight: 600, color: "#1a1a1a" }}>Choose a file or drag & drop it here</p>
//           <p style={{ fontSize: 11, color: "#71717a", marginTop: 2 }}>PDF, DOC, TXT files</p>
//           <button style={{ marginTop: 4, padding: "7px 16px", borderRadius: 8, border: "1px solid #d4d4d8", background: "#fff", fontSize: 12, fontWeight: 500, color: "#1a1a1a", cursor: "pointer" }}>
//             Browse Files
//           </button>
//         </>
//       )}
...
// }) {
//   return (
//     <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto auto", gap: 8, alignItems: "center", background: "#fafafa", borderRadius: 10, padding: "10px 12px", border: "1px solid #e4e4e7" }}>
//       <div style={{ position: "relative" }}>
//         <select value={config.type} onChange={e => onUpdate(index, "type", e.target.value)}
//           style={{ width: "100%", padding: "8px 32px 8px 12px", fontSize: 13, border: "1px solid #e4e4e7", borderRadius: 8, background: "#fff", color: "#1a1a1a", appearance: "none", cursor: "pointer", fontFamily: "inherit", outline: "none" }}>
...
//         <button onClick={() => onUpdate(index, "count", Math.max(1, config.count - 1))}
//           style={{ width: 26, height: 26, borderRadius: 6, border: "1px solid #e4e4e7", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
//           <Minus size={11} />
//         </button>
//         <input type="number" value={config.count} min={1} max={50}
//           onChange={e => onUpdate(index, "count", Math.max(1, parseInt(e.target.value) || 1))}
//           style={{ width: 40, textAlign: "center", fontSize: 13, fontWeight: 600, border: "1px solid #e4e4e7", borderRadius: 6, padding: "4px 2px", outline: "none", fontFamily: "inherit" }} />
//         <button onClick={() => onUpdate(index, "count", Math.min(50, config.count + 1))}
//           style={{ width: 26, height: 26, borderRadius: 6, border: "1px solid #e4e4e7", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
//           <Plus size={11} />
//         </button>
//       </div>
...
//       <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
//         <button onClick={() => onUpdate(index, "marksPerQuestion", Math.max(1, config.marksPerQuestion - 1))}
//           style={{ width: 26, height: 26, borderRadius: 6, border: "1px solid #e4e4e7", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
//           <Minus size={11} />
//         </button>
//         <input type="number" value={config.marksPerQuestion} min={1} max={20}
//           onChange={e => onUpdate(index, "marksPerQuestion", Math.max(1, parseInt(e.target.value) || 1))}
//           style={{ width: 40, textAlign: "center", fontSize: 13, fontWeight: 600, border: "1px solid #e4e4e7", borderRadius: 6, padding: "4px 2px", outline: "none", fontFamily: "inherit" }} />
//         <button onClick={() => onUpdate(index, "marksPerQuestion", Math.min(20, config.marksPerQuestion + 1))}
//           style={{ width: 26, height: 26, borderRadius: 6, border: "1px solid #e4e4e7", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
//           <Plus size={11} />
//         </button>
//       </div>
...
//         </div>
//       </div>
//       <h2 style={{ fontSize: 20, fontWeight: 700, color: "#1a1a1a", marginBottom: 8 }}>Generating Question Paper</h2>
//       <p style={{ fontSize: 14, color: "#71717a", marginBottom: 24, maxWidth: 320 }}>{message}</p>
//       <div style={{ width: 300, height: 6, background: "#e4e4e7", borderRadius: 999, overflow: "hidden" }}>
//         <div style={{ height: "100%", borderRadius: 999, background: "linear-gradient(90deg, #f97316, #ea580c)", width: `${progress}%`, transition: "width 0.5s ease" }} />
//       </div>
//       <p style={{ fontSize: 12, color: "#a1a1aa", marginTop: 16 }}>Please wait while AI creates your question paper...</p>
//     </div>
//   );
// }
...
//   // ✅ updateConfig defined here — inside component
...
//           if (assignment.status === "processing") {
//             setProgress(Math.min(20 + attempts * 4, 85));
//             setProgressMsg("AI is generating your question paper...");
//           } else {
//             setProgress(Math.min(15 + attempts * 2, 40));
...
//       <AppShell title="Create Assignment" showBack={false}>
//         <div style={{ maxWidth: 600, margin: "0 auto", padding: "40px 20px" }}>
//           <GeneratingScreen progress={progress} message={progressMsg} />
//         </div>
//       </AppShell>
//     );
//   }
...
//               <div style={{ marginBottom: 20 }}>
//                 <FileUpload file={formData.file} onFile={(f) => setFormData({ file: f })} />
//                 <p style={{ fontSize: 11, color: "#71717a", marginTop: 6, textAlign: "center" }}>Upload images of your preferred document/image</p>
//               </div>

//               <div style={{ marginBottom: 14 }}>
//                 <label style={{ fontSize: 12, fontWeight: 600, color: "#1a1a1a", display: "block", marginBottom: 6 }}>Assignment Title *</label>
//                 <input type="text" placeholder="e.g. Science Chapter 5 Quiz" value={formData.title}
//                   onChange={e => setFormData({ title: e.target.value })}
//                   style={{ width: "100%", padding: "10px 14px", fontSize: 13, border: `1px solid ${errors.title ? "#fca5a5" : "#e4e4e7"}`, borderRadius: 10, outline: "none", fontFamily: "inherit", background: "#fafafa" }} />
//                 {errors.title && <p style={{ fontSize: 11, color: "#ef4444", marginTop: 4 }}>{errors.title}</p>}
//               </div>
...
//                 <div>
//                   <label style={{ fontSize: 12, fontWeight: 600, color: "#1a1a1a", display: "block", marginBottom: 6 }}>Grade/Class *</label>
//                   <input type="text" placeholder="e.g. Grade 8" value={formData.grade}
//                     onChange={e => setFormData({ grade: e.target.value })}
//                     style={{ width: "100%", padding: "10px 14px", fontSize: 13, border: `1px solid ${errors.grade ? "#fca5a5" : "#e4e4e7"}`, borderRadius: 10, outline: "none", fontFamily: "inherit", background: "#fafafa" }} />
//                 </div>
//               </div>
...
//               <div style={{ marginBottom: 14 }}>
//                 <label style={{ fontSize: 12, fontWeight: 600, color: "#1a1a1a", display: "block", marginBottom: 6 }}>Topic *</label>
//                 <input type="text" placeholder="e.g. Electromagnetism" value={formData.topic}
//                   onChange={e => setFormData({ topic: e.target.value })}
//                   style={{ width: "100%", padding: "10px 14px", fontSize: 13, border: `1px solid ${errors.topic ? "#fca5a5" : "#e4e4e7"}`, borderRadius: 10, outline: "none", fontFamily: "inherit", background: "#fafafa" }} />
//               </div>
...
//                 <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
//                   {formData.questionConfigs.map((config, i) => (
//                     <QuestionRow key={i} index={i} config={config}
...
//                 <button onClick={addQuestionConfig}
//                   style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10, fontSize: 12, fontWeight: 600, color: "#1a1a1a", background: "none", border: "none", cursor: "pointer", padding: "6px 4px" }}>
//                   <div style={{ width: 20, height: 20, borderRadius: "50%", background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center" }}>
//                     <Plus size={12} color="#fff" />
//                   </div>
//                   Add Question Type
...
//               <div style={{ display: "flex", gap: 16, justifyContent: "flex-end", padding: "10px 0", borderTop: "1px solid #f4f4f5", marginBottom: 16 }}>
//                 <span style={{ fontSize: 12, color: "#71717a" }}>Total Questions : <strong style={{ color: "#1a1a1a" }}>{totalQuestions}</strong></span>
//                 <span style={{ fontSize: 12, color: "#71717a" }}>Total Marks : <strong style={{ color: "#1a1a1a" }}>{totalMarks}</strong></span>
//               </div>
...
//                     onChange={e => setFormData({ additionalInstructions: e.target.value })}
//                     rows={3}
//                     style={{ width: "100%", padding: "10px 40px 10px 14px", fontSize: 13, border: "1px solid #e4e4e7", borderRadius: 10, outline: "none", fontFamily: "inherit", background: "#fafafa", resize: "vertical", lineHeight: 1.5 }} />
//                   <Mic size={15} color="#71717a" style={{ position: "absolute", right: 14, bottom: 14 }} />
//                 </div>
//               </div>
//             </>
...
//               {[["Title", formData.title], ["Subject", formData.subject], ["Grade", formData.grade], ["Topic", formData.topic], ["Due Date", formData.dueDate], ["File", formData.file?.name || "None"]].map(([label, value]) => (
//                 <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #f4f4f5" }}>
//                   <span style={{ fontSize: 13, color: "#71717a", fontWeight: 500 }}>{label}</span>
//                   <span style={{ fontSize: 13, color: "#1a1a1a", fontWeight: 600, textAlign: "right", maxWidth: 300 }}>{value}</span>
//                 </div>
...
//                 {formData.questionConfigs.map((c, i) => (
//                   <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: "#fafafa", borderRadius: 8, marginBottom: 6 }}>
//                     <span style={{ fontSize: 12, color: "#1a1a1a" }}>{QUESTION_TYPE_OPTIONS.find(o => o.value === c.type)?.label}</span>
//                     <span style={{ fontSize: 12, color: "#71717a" }}>{c.count} Q × {c.marksPerQuestion} marks</span>
//                   </div>
//                 ))}
//               </div>
...
//             style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 10, border: "1px solid #e4e4e7", background: "#fff", fontSize: 13, fontWeight: 600, color: "#1a1a1a", cursor: "pointer" }}>
//             <ArrowLeft size={15} />
//             Previous
//           </button>
...
//               style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 24px", borderRadius: 10, border: "none", background: "#1a1a1a", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer" }}>
//               Next <ArrowRight size={15} />
//             </button>
//           ) : (
//             <button onClick={handleSubmit} disabled={submitting}
//               style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 24px", borderRadius: 10, border: "none", background: submitting ? "#a1a1aa" : "#f97316", fontSize: 13, fontWeight: 600, color: "#fff", cursor: submitting ? "not-allowed" : "pointer" }}>
//               {submitting ? "Creating..." : "Generate Paper"} {!submitting && <ArrowRight size={15} />}
//             </button>
//           )}
//         </div>
//       </div>
